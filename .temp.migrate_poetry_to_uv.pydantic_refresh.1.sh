...
REPO="/home/jon/Work/AjanCodesExamples/2024/pydantic_refresh"
...
echo "[pydantic_refresh] migration started: $(date -Is)" >> "$LOG"

UV_CACHE_DIR="$CACHE_DIR" uv sync --refresh

# Run deptry to audit dependencies
if ! UV_CACHE_DIR="$CACHE_DIR" uv run deptry .; then
  echo "[pydantic_refresh] deptry audit failed - missing or unused dependencies detected" >> "$LOG"
  exit 1
fi

if grep -q "\[dependency-groups\]" pyproject.toml && grep -q "dev\s*=\s*" pyproject.toml; then
  UV_CACHE_DIR="$CACHE_DIR" uv sync --group dev || echo "[pydantic_refresh] uv sync --group dev failed" >> "$LOG"
fi
...
if ! "$PYTHON_BIN" -m ruff check .; then
  echo "[pydantic_refresh] ruff check failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m mypy "${MYPY_TARGETS[@]}"; then
  echo "[pydantic_refresh] mypy failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m pytest; then
  echo "[pydantic_refresh] pytest failed" >> "$LOG"
fi
...
echo "[pydantic_refresh] migration complete: $(date -Is)" >> "$LOG"
