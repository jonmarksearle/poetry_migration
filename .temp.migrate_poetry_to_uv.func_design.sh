...
echo "[func_design] migration started: $(date -Is)" >> "$LOG"

UV_CACHE_DIR="$CACHE_DIR" uv sync --refresh

if grep -q "\[dependency-groups\]" pyproject.toml && grep -q "dev\s*=\s*" pyproject.toml; then
  UV_CACHE_DIR="$CACHE_DIR" uv sync --group dev || echo "[func_design] uv sync --group dev failed" >> "$LOG"
fi

PYTHON_BIN=".venv/bin/python"

if ! "$PYTHON_BIN" -m ruff check .; then
  echo "[func_design] ruff check failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m mypy .; then
  echo "[func_design] mypy failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m pytest; then
  echo "[func_design] pytest failed" >> "$LOG"
fi

echo "[func_design] migration complete: $(date -Is)" >> "$LOG"
