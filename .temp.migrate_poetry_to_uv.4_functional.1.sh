...
REPO="/home/jon/Work/AjanCodesExamples/2024/funky/4_functional"
...
echo "[4_functional] migration started: $(date -Is)" >> "$LOG"

UV_CACHE_DIR="$CACHE_DIR" uv sync --refresh

# Run deptry to audit dependencies
if ! UV_CACHE_DIR="$CACHE_DIR" uv run deptry .; then
  echo "[4_functional] deptry audit failed - missing or unused dependencies detected" >> "$LOG"
  exit 1
fi

if grep -q "\[dependency-groups\]" pyproject.toml && grep -q "dev\s*=\s*" pyproject.toml; then
  UV_CACHE_DIR="$CACHE_DIR" uv sync --group dev || echo "[4_functional] uv sync --group dev failed" >> "$LOG"
fi
...
if ! "$PYTHON_BIN" -m ruff check .; then
  echo "[4_functional] ruff check failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m mypy "${MYPY_TARGETS[@]}"; then
  echo "[4_functional] mypy failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m pytest; then
  echo "[4_functional] pytest failed" >> "$LOG"
fi
...
echo "[4_functional] migration complete: $(date -Is)" >> "$LOG"
