...
REPO="/home/jon/Work/AjanCodesExamples/2024/funky/1_oo"
...
echo "[1_oo] migration started: $(date -Is)" >> "$LOG"

UV_CACHE_DIR="$CACHE_DIR" uv sync --refresh

# Run deptry to audit dependencies
if ! UV_CACHE_DIR="$CACHE_DIR" uv run deptry .; then
  echo "[1_oo] deptry audit failed - missing or unused dependencies detected" >> "$LOG"
  exit 1
fi

if grep -q "\[dependency-groups\]" pyproject.toml && grep -q "dev\s*=\s*" pyproject.toml; then
  UV_CACHE_DIR="$CACHE_DIR" uv sync --group dev || echo "[1_oo] uv sync --group dev failed" >> "$LOG"
fi
...
if ! "$PYTHON_BIN" -m ruff check .; then
  echo "[1_oo] ruff check failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m mypy "${MYPY_TARGETS[@]}"; then
  echo "[1_oo] mypy failed" >> "$LOG"
fi

if ! "$PYTHON_BIN" -m pytest; then
  echo "[1_oo] pytest failed" >> "$LOG"
fi
...
echo "[1_oo] migration complete: $(date -Is)" >> "$LOG"
